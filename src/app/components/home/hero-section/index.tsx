    ...
    const socialIcon = [
        {
            ...
            href: "https://linkedin.com/in/gopika-s-6874a330b/",
            icon: "LinkedIn"
        },
        {
            img: "/images/icon/mail-icon.svg",
            href: "mailto:gopika.s2310314@gmail.com",
            icon: "Email"
        },
        {
            img: "/images/icon/phone-icon.svg",
            href: "tel:+917200593708",
            icon: "Phone"
        },
    ];
    return (
                    ...
                    <div className="border-x border-primary/10">
                        <div className="relative flex flex-col xs:flex-row items-center xs:items-start justify-center xs:justify-between max-w-3xl mx-auto gap-10 xs:gap-3 px-4 sm:px-7 pt-22 pb-8 sm:pb-12">
                            <div className="absolute top-0 transform -translate-y-1/2">
                                <Image src={"/images/hero-sec/user-img.svg"} alt="Gopika S" width={145} height={145} className="border-4 border-white rounded-full" />
                                <span className="absolute bottom-2.5 right-5 w-4 h-4 bg-green-500 border-2 border-white rounded-full" />
                            </div>
                            <div className="flex flex-col gap-2 sm:gap-3 items-center text-center xs:items-start">
                                <h1>Gopika S</h1>
                                <p className="text-violet-700 font-normal">B.E. Computer Science & IoT | Software Enthusiast</p>
                                <div className="flex items-center gap-2">
                                    <Image src={"/images/icon/map-icon.svg"} alt="map-icon" width={20} height={20} />
                                    <p className="text-primary">Madurai, Tamil Nadu</p>
                                </div>
                            </div>
                            <div className="flex flex-col md:flex-row items-center gap-4">
                                <div className="flex items-center gap-2">
                                    {socialIcon?.map((value, index) => {
                                        return (
                                            <Link href={value?.href} key={index} target="_blank" className="w-fit p-2.5 sm:p-3.5 hover:bg-primary/5 border border-primary/10 rounded-full">
